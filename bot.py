...
DEBUG_PROJECT = 'VK/fmtn/fmtntas'

IMPORTANT_BRANCHES = ['dev', 'master', 'frontend-dev']
FIXED_STATUSES = ['canceled', 'success', 'failed']
...
def get_project_state(gl, gl_project):
    ...
    gl_pipelines = gl_project.pipelines.list(all=True)

    for gl_pipeline in gl_pipelines:
        ref = gl_pipeline.ref
        # ignoring pipelines without ref and pipelines with pending statuses
        if (ref != None and gl_pipeline.status in FIXED_STATUSES and ref in IMPORTANT_BRANCHES):
                ...
                for ref, n_pipeline_state in n_project_state.pipeline_states.items():
                    ...
                    if (o_pipeline_state != None):
                        ...
                        if (not old_pipeline_failed or new_pipeline) and new_pipeline_failed:
                            events.append(PipelineEvent(PipelineEventType.FAILED, project_name, ref, n_pipeline_state.url, n_pipeline_state.id))
                        elif old_pipeline_failed and not new_pipeline_failed:
                            events.append(PipelineEvent(PipelineEventType.RESTORED, project_name, ref, n_pipeline_state.url, n_pipeline_state.id))
    ...
    try:
        for event in events:
            if event.event_type == PipelineEventType.FAILED:
                push_message_to_every_chat("\U000026A0  Pipeline [{}]({}) failed for project *{}* branch *{}*.".format(event.pipeline_id, event.url, event.project_name, event.ref))   
                
            elif event.event_type == PipelineEventType.RESTORED:
                push_message_to_every_chat("\U00002705  Pipeline [{}]({}) restored for project *{}* branch *{}*.".format(event.pipeline_id, event.url, event.project_name, event.ref))  
    except Exception as e:
        print("ERROR: failed to find interesting events {}".format(e))    
        traceback.print_exc()   
...
def trottled_sending(bot, job):
    ...
    if not messages.empty():
        ...
        if not message is None:
            bot.send_message(chat_id = message.chat_id, text = message.text, parse_mode = 'Markdown')
...
def hello(bot, update):
    chat_id = update.message.chat.id
    push_message_to_chat(chat_id, '\U00002705  hello [example link](https://example.com)')
...
def check_pipelines_job():
    try:
        ...
        threading.Timer(30, check_pipelines_job).start()
...
def main():
    ...
    try:
        ...
        updater.job_queue.run_repeating(trottled_sending, interval=1, first=0)
